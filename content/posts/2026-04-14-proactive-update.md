---
title: "에이전트 메모리 설계: 장기 기억, 검색, 삭제 전략"
date: 2026-04-14
tags:
  - AI Agents
  - Agent Memory
  ...
  - System Design
  - LLM
description: "단기·중기·장기 기억을 분리하고 임베딩 검색과 삭제 전략을 연결하는 방법을 아키텍처와 코드 예시로 다룹니다."
---
...
2026년 4월 현재, 프로덕션 Agent 시스템에서 memory 설계 미숙으로 인한 실패는 **task hallucination**(이전 결과를 사실로 착각), **컨텍스트 간 정보 누출**(이전 대화의 민감 정보가 새 대화에 누출), **과도한 비용**(불필요한 긴 컨텍스트로 토큰 비용 폭증)으로 나타난다.
...
"서울 출장 기억해?"와 "작년 3월 가던 카페 기억해?"는 벡터 공간에서 멀다. 시간, 장소, 감정 등의 메타데이터는 별도 필터로 관리해야 한다.
...
## 5. '잊어버림(Forget)' 전략: 메모리를 무한히 늘릴 때의 대가

### 왜 Forget이 중요한가

LLM Context는 유한 자원이므로, "기억이 쌓일수록 새 기억이 더 잘 들어간다"는 착각이 있다. 실제로는 **중요도가 낮은 오래된 기억이 중요한 새 기억의 공간을 차지**하여 성능을 저하시킨다.
...
Agent Memory 시스템은 단순히 "대화를 저장하는 배열"이 아니다. **단기 / 중기 / 장기의 분리**, **계층적 요약**, **하이브리드 검색**, **의도적인 forget**이 결합된 복잡한 하위 시스템이다.
...
2. **Forget은 설계다**: 모든 기억을 저장하는 것은 메모리 하위 시스템의 확장성을 해친다. Importance decay, TTL, semantic deduplication을 처음부터 설계에 넣어야 한다.
